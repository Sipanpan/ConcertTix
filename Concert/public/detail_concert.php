...
// Fungsi untuk menambahkan item ke cart
// Fungsi untuk menambahkan item ke cart (dengan dukungan Promise)
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(result => {
      if (result.success) {
        // Update cart count
        updateCartCount();
        resolve(result); // Resolve promise jika sukses
      } else {
        reject(new Error(result.message || 'Failed to add item to cart'));
      }
    })
    .catch(error => {
      console.error('Error:', error);
...
// Event listener khusus untuk Buy Now
// Event listener khusus untuk Buy Now
document.addEventListener('click', function(e) {
  // Pastikan yang diklik adalah tombol Buy Now secara spesifik
  if (e.target.closest('.btn-book') && e.target.closest('.btn-book').textContent.includes('Buy Now')) {
    ...
    addToCart(ticketTypeId, quantity)
      .then(() => {
        // 2. Redirect ke halaman payment
        window.location.href = '../user/payment/index.php'; 
      })
      .catch(error => {
...
document.addEventListener('click', function(e) {
  // Pastikan yang diklik adalah tombol Add to Cart secara spesifik
  if (e.target.closest('.btn-book') && e.target.closest('.btn-book').textContent.includes('Add to Cart')) {
    const button = e.target.closest('.btn-book');
    const ticketOption = button.closest('.ticket-option');
    const ticketTypeId = ticketOption.dataset.ticketTypeId;
    const quantityInput = ticketOption.querySelector('.quantity-input');
